...
from pistehaku import pistehaku
...
def main():
    ...
    nappi_wrapping = tk.Button(ikkuna, text = "Gift Wrapping", width=25, command= lambda: dialogi(int(syote.get()), "Gift Wrapping ", 1))
    nappi_graham = tk.Button(ikkuna, text = "Graham Scan", width=25, command= lambda: dialogi(int(syote.get()), "Graham Scan ", 2))
    nappi_quickhull = tk.Button(ikkuna, text = "Quickhull", width=25, command= lambda: dialogi(int(syote.get()), "Quickhull", 3))
...
def dialogi(maara, nimi, numero):
    print("Suoritetaan " + nimi + "-algoritmi " + str(maara) + ":n pisteen joukolle")
    ...
    print(str(maara) + " pisteellä " + nimi + "-algortimilla meni x millisekuntia. (WIP)")

def pisteiden_haku(maara):
    return pistehaku(maara)
